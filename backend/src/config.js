...
const config = {
    ...
    ocrEnabled: booleanEnvironment("OCR_ENABLED", false),
    ocrProvider: process.env.OCR_PROVIDER || "openai",
    openAiOcrModel: process.env.OPENAI_MODEL_OCR || null,
    ocrMaxImageBytes: numberEnvironment("OCR_MAX_IMAGE_BYTES", 8 * 1024 * 1024),
    ocrMaxPdfPages: numberEnvironment("OCR_MAX_PDF_PAGES", 10),
    ocrMaxTotalBytes: numberEnvironment("OCR_MAX_TOTAL_BYTES", 30 * 1024 * 1024),
    ocrTimeoutMs: numberEnvironment("OCR_TIMEOUT_MS", 60000),
    ocrMaxConcurrentRequests: numberEnvironment("OCR_MAX_CONCURRENT_REQUESTS", 1),
    ocrRateLimitWindowMs: numberEnvironment("OCR_RATE_LIMIT_WINDOW_MS", 60 * 60 * 1000),
    ocrRateLimitMaxRequests: numberEnvironment("OCR_RATE_LIMIT_MAX_REQUESTS", 10),
