...
function row(title, detail, score, href = null) {
    ...
    element.querySelector(".progress-score").textContent = score;
    if (href) {
        element.classList.add("clickable-progress-row"); element.tabIndex = 0; element.setAttribute("role", "link");
...
function render(data) {
    ...
    data.recentActivity.forEach(item => activity.appendChild(row(`${item.courseCode} · Quiz #${item.quizId}`, `${formatDate(item.createdAt)} · ${item.questionCount || "—"} questions`, `${item.score}%`, StudyAI.courseContext.url("quiz.html", { courseId: item.courseId, quizId: item.quizId }))));
}
async function loadProgress() {
    const selected = Number(progressSelect.value);
    const path = selected ? `/api/courses/${selected}/progress` : "/api/progress";
    document.querySelector("#progress-error").textContent = "";
...
async function initializeProgress() {
    try {
        ...
        const action = document.querySelector("#progress-empty-action");
        if (requestedCourseId) { action.href = StudyAI.courseContext.url("quiz.html", { courseId: requestedCourseId }); action.textContent = "Start a Practice Quiz"; }
        await loadProgress();
    } catch (error) { document.querySelector("#progress-error").textContent = error.message; }
}
progressSelect.addEventListener("change", () => { const value = Number(progressSelect.value); window.history.replaceState(null, "", value ? `progress.html?courseId=${value}` : "progress.html"); const action = document.querySelector("#progress-empty-action"); action.href = value ? StudyAI.courseContext.url("quiz.html", { courseId: value }) : "index.html#courses"; action.textContent = value ? "Start a Practice Quiz" : "Choose a Course"; loadProgress(); });
initializeProgress();
